function net = res_init(n, varargin)
...
if numel(n)==4, 
  ...
  switch n, 
    case 10, Ns = [1 1 1 1]; opts.bottleneck = false; 
    case 18, Ns = [2 2 2 2]; opts.bottleneck = false; 
    case 20, Ns = [2 3 2 2]; opts.bottleneck = false;
    case 30, Ns = [3 4 4 3]; opts.bottleneck = false;
    case 34, Ns = [3 4 6 3]; opts.bottleneck = false; 
    case 40, Ns = [3 5 7 4]; opts.bottleneck = false;
    case 50, Ns = [3 4 6 3]; opts.bottleneck = true;
    case 80, Ns = [3 4 16 3]; opts.bottleneck = true;
...
if opts.batchNormalization; 
   net.meta.trainOpts.learningRate = [0.001*ones(1,50) 0.0001*ones(1,100) 0.00001*ones(1,50)] ;
else
   net.meta.trainOpts.learningRate = [0.001*ones(1,50) 0.0001*ones(1,100) 0.00001*ones(1,50)] ;
end
...
% add a filter layer
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
block = dagnn.Conv('size',  [5 5 1 3], 'hasBias', false, ...
...
% First conv layer
switch n
    case {10, 18, 20, 30, 34, 40, 50}
        block = dagnn.Conv('size',  [7 7 3 64], 'hasBias', true, ...
            'stride', 2, 'pad', [3 3 3 3]);
        lName = 'conv0';
net.addLayer(lName, block, 'filter_data', lName, {[lName '_f'], [lName '_b']});
    case {80, 101, 152}
        block = dagnn.Conv('size',  [7 7 3 64], 'hasBias', false, ...
            'stride', 2, 'pad', [3 3 3 3]);
        lName = 'conv0';
    net.addLayer(lName, block, 'filter_data', lName, {[lName '_f']});
end
...
block = dagnn.Pooling('poolSize', [5 5], 'method', 'avg', 'pad', [0 2 0 2], 'stride', 2); 
