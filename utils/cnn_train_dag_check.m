...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    %[stats.train(epoch),prof] = process_epoch(net, state, opts, 'train',epoch) ;
    stats.val(epoch) = process_epoch(net, state, opts, 'val',epoch) ;
    if opts.profile
      profview(0,prof) ;
      keyboard ;
    end
...
% -------------------------------------------------------------------------
function [stats, prof] = process_epoch(net, state, opts, mode, epoch)
% -------------------------------------------------------------------------
...
for p=1:numel(net.params)
  %num = state.epoch;
  %if p == 1
  %  net.params(p).learningRate = 0.0001/(num);
  %end
